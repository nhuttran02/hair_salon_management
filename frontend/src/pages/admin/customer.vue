<template>
    <div class="useradmin-container">
        <div class="mb-4 flex justify-between items-center">
            <h1 class="text-2xl px-5">CUSTOMER ADMINISTRATION</h1>
        ...
        <!-- No Data State -->
        <div v-else-if="!loading" class="text-center py-4 bg-white rounded-lg shadow">No customers found.</div>
                ...
                <form @submit.prevent="showAddModal ? addUser() : updateUser()">
                    ...
                    <div class="flex justify-end">
                        ...
                        <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600">{{
                            showAddModal ? 'Add Customer' : 'Update Customer' }}</button>
                    </div>
                </form>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        //     this.error = null;
        //     try {
        //         const response = await axios.get(`/api/v1/users?page=${this.currentPage}&role=2`);
        ...
        async fetchUsers() {
            ...
            this.error = null;
            try {
                // Thêm tham số filter user_role === 2 vào API request
                const response = await axios.get(`/api/v1/users?page=${this.currentPage}&user_role=2`);
                const data = response.data;
                this.users = data.data.users;  // Chỉ lấy users có user_role === 2
                this.metadata = data.data.metadata;
            } catch (err) {
        ...
        async addUser() {
            try {
                // Mặc định vai trò của người dùng là Customer
                const userForm = {
                    ...this.form,
                    user_role: 2,
                };
