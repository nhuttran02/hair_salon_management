            ...
            <div class="bg-white p-6 rounded-lg w-96">
                ...
                <form @submit.prevent="showAddModal ? addUser() : updateUser()">
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="form.user_role" id="role" :required="showAddModal" style="padding-left: 0.5rem;"
                            class="mt-1 py-1 block w-full border border-gray-300 rounded shadow-sm focus:ring focus:ring-green-500">
...
export default {
    ...
    methods: {
        async fetchUsers() {
            ...
            this.error = null;
            try {
                const response = await fetch(`/api/v1/users?page=${this.currentPage}`);
        ...
        async updateUser() {
            try {
                const response = await fetch(`/api/v1/users/${this.form.user_id}`, {
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                    },
